...
export default function ElasticPath() {
  return null;
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    return await authenticator.authenticate("elastic-path", request, {
    ...
    console.error({
      error,
      extras: {
        loginMethod: AUTH_PROVIDERS.LOGIN_ELASTIC_PATH,
      },
    });

    return redirect(
      loginPath({
        error: AUTH_PROVIDERS.LOGIN_ELASTIC_PATH,
        message: message,
        returnTo,
      })
    );
